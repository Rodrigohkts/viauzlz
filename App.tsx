...
export const App: React.FC = () => {
    const [hasApiKey, setHasApiKey] = useState(true); // Hardcoded so it starts true
    const [isMobile, setIsMobile] = useState(false);
    const [customKeyInput, setCustomKeyInput] = useState('');

    // Check for API Key on mount (Gemini)
    useEffect(() => {
        // Automatically assume true since the API key is hardcoded in geminiService.ts
        setHasApiKey(true);
    }, []);
    ...
    const handleCustomBgGenerate = async () => {
        if (!resultImage || isBusy || !checkCredits(COST_VARIATION) || !customBgPrompt) return;
        setStatus(ProcessingStatus.REFINING);
        try {
            const instruction = `substitua o fundo por: ${customBgPrompt}. Mantenha a pessoa e a roupa intactas.`;
            const refined = await refineImage(resultImage, instruction);
            deductCredits(COST_VARIATION);
            setFinalResultImage(refined);
            setStatus(ProcessingStatus.SUCCESS);
        } catch (error: any) {
                ...
                <div className="relative w-[2200px] h-[1400px]">
                    ...
                    {showExtendedFlow && (
                        <>
                            ...
                            <NodeCard title="Fundo Personalizado" type="text-input" textValue={customBgPrompt} onTextChange={(e) => setCustomBgPrompt(e.target.value)} onEnhance={handleCustomBgGenerate} isEnhancing={status === ProcessingStatus.REFINING} style={{ left: positions.bgEditor.x, top: positions.bgEditor.y }} onDragStart={(e) => handleDragStart(e, 'bgEditor', positions.bgEditor.x, positions.bgEditor.y)} onTouchStart={(e) => handleTouchStart(e, 'bgEditor', positions.bgEditor.x, positions.bgEditor.y)} isMobile={isMobile} />
            ...
            <div className="absolute top-6 left-1/2 -translate-x-1/2 h-16 bg-white/5 backdrop-blur-2xl border border-white/10 flex items-center px-4 rounded-3xl z-[60] shadow-2xl ring-1 ring-white/5">
                ...
                <div className="flex items-center p-1.5 gap-1 overflow-x-auto max-w-[50vw] md:max-w-none custom-scrollbar">
                    {[
                        ...
                        { id: 'flow', label: 'Roupa & Fundo', icon: <Plus size={14} /> },
